...
DEFAULT_RULEBOOK_PATH = os.environ.get("RHK_RULEBOOK", os.path.join(APP_DIR, "rhk_rules_v19.yaml"))
...
def build_case(ui: Dict[str, Any], rules: List[Rule]) -> Dict[str, Any]:
    ...
    ci_peak = None
    if co_peak is not None and bsa is not None:
        ci_peak = co_peak / bsa
    ...
    tapse_spap = None
    tapse_spap_uncoupled = None
    # Cut-off nach Tello et al.: TAPSE/sPAP < 0.31 mm/mmHg spricht für RV-PA Entkopplung
    TAPSE_SPAP_CUTOFF = 0.31
...
def build_render_ctx(case: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sbp = _safe_float(ui.get("sbp_mmHg"))
    hr = _safe_float(ui.get("hr_bpm"))
    systemic_sentence = ""
    ...
    anemia_context_desc = "Anämie"
    if der.get("anemia"):
        at = ui.get("anemia_type")
        if at == "microcytic":
            anemia_context_desc = "Mikrozytäre Anämie (Eisenmangel wahrscheinlich)"
        elif at == "macrocytic":
            anemia_context_desc = "Makrozytäre Anämie (B12/Folat/Leber/Alkohol DD)"
        elif at == "normocytic":
            anemia_context_desc = "Normozytäre Anämie (Entzündung/Niere/Blutverlust DD)"
...
def summarize_inputs(case: Dict[str, Any]) -> str:
    ...
    # symptoms
    if ui.get("syncope") is True:
        klinik_lines.append(_md_kv("Synkope", "ja"))
...
def build_doctor_report(case: Dict[str, Any], blocks: Dict[str, TextBlock]) -> str:
    ...
    if recs:
        report.append("**Zusätzliche Hinweise (Regelwerk):**\n" + "\n".join([f"- {r}" for r in recs]) + "\n")
...
def random_example() -> Dict[str, Any]:
    ...
    ui["syncope"] = random.choice([False, False, True])
    ui["hemoptysis"] = False
    ...
    if "anemia" in lab_mode:
        ...
        ui["anemia_type"] = "microcytic" if lab_mode == "anemia_micro" else "macrocytic"
    else:
...
def build_demo() -> Tuple[gr.Blocks, str, gr.Theme]:
    ...
    with gr.Blocks(title=APP_TITLE) as demo:
            ...
            with gr.Column(scale=7):
                ...
                with gr.TabItem("Klinik & Labor", id=0):
                    ...
                    with gr.Row():
                        ...
                        add("syncope", gr.Checkbox(label="Synkope"))
                    with gr.Row():
                    ...
                    with gr.Row():
                        ...
                        anemia_type = add("anemia_type", gr.Dropdown(
                            label="Anämie-Typ (falls relevant)",
                            choices=["microcytic", "normocytic", "macrocytic"],
                            value=None,
                            visible=False,
                        ))
                    with gr.Row():
        ...
        def _clear():
            ...
            return (*vals, None, None, None, None, None, dash, "", "", "", "{}", None)
